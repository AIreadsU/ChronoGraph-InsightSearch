    ...
    try:
        if request.method == 'POST':
            try:
                ...
                logger.info("开始处理文件上传，文件上传参数验证通过")
                ...
                logger.error(f"在处理文件上传过程中出现异常: {e}", exc_info=True)
                return JsonResponse({'error': 'An internal error occurred'}, status=500)
    except Exception as e:
        logger.error(f"在 upload_file 视图函数入口处出现异常: {e}", exc_info=True)
        return JsonResponse({'error': 'An internal error occurred'}, status=500)
@csrf_exempt
