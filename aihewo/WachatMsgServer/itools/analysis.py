...
import jieba
from .WXDateBase import msg_db, MsgType
#from WXDateBase import micro_msg_db,msg_db,MsgType
from pyecharts import options as opts
from pyecharts.charts import WordCloud, Calendar, Bar, Line, Pie, Map
from .util.region_conversion import conversion_province_to_chinese
#from util.region_conversion import conversion_province_to_chinese
...
def wordcloud_(wxid, time_range=None):
    ...
    stopwords_file = os.path.join(BASE_DIR,
                           'WachatMsgServer',
                           'itools', 'WXDateBase',
                           'data', 'stopwords.txt')
...
def get_wordcloud(text):
    total_msg_len = len(text)
    new_words_file = os.path.join(BASE_DIR,
                                  'WachatMsgServer',
                                  'itools', 'WXDateBase',
                                  'data', 'new_words.txt')
    # jieba.load_userdict('./app/data/new_words.txt')
    jieba.load_userdict(new_words_file)
    # 使用jieba进行分词，并加入停用词
    words = jieba.cut(text)
    # 统计词频
    word_count = Counter(words)
...
def my_message_counter(wxid, time_range):
    ...
    if not data:
        return {
            ...
            'chart_data_types': None,
            'chart_data_weekday': None,
            'chart_data_wordcloud': None,
            'keyword': None,
            'keyword_max_num': None,
            'total_text_num': None,
        }
    p1 = (
        Pie()
...
if __name__ == '__main__':
    db_pathdate = os.path.join(BASE_DIR,
                           'WachatMsgServer',
                           'itools', 'WXDateBase',
                           'Database', 'Msg', '2024-10-16_22-15-20','wxid_yzy19wfvnozj21','MSG.db')
    print(db_pathdate)
    msg_db.init_database(path=db_pathdate)
